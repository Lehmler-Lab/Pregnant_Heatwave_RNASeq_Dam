...
```{r}
se1<- se[,se$tissue_type == "hippocampus"] ##change tissue type here
...
#PCA Plot
```{r}
tiff(file = 'Figures/PCA/Female_Hippocampus_PCA.tiff', units="in", width=8, height=5, res=1000)
PCA <-ggplot(data, aes(PC1, 
                   PC2, 
                   color=exposure,
                   shape= exposure)) + 
  stat_ellipse(aes(color= exposure))+
  ggtitle("Female Hippocampus PCA")+
  geom_point(size=3) +
  scale_color_manual(values = c("#666666", "#FF66CC","#66CCCC")) +
...
```{r}
res_exp<-results(dds, name = "exposure_exposed_vs_control")
res_pf<-results(dds, name = "exposure_pair_fed_vs_control")
...
```{r}
res_pf$symbol <- mapIds(Mus.musculus, #which annotation package model to use
                     keys=row.names(res_pf), #using rownames
                     column="SYMBOL", #add a column of symbols associated with row.names arguments for the model
                     keytype="ENSEMBL", #our rownames are gene id's 
                     multiVals="first")
```
...
```{r}
res_pf$ONTOLOGY <- mapIds(Mus.musculus,
                     keys=row.names(res_pf),
                     column="ONTOLOGYALL",
                     keytype="ENSEMBL",
                     multiVals="first")
```
...
```{r}
res_pf$Genename <- mapIds(Mus.musculus,
                     keys=row.names(res_pf),
                     column="GENENAME", #now add a column for gene names or gene description
                     keytype="ENSEMBL",
                     multiVals="first")
...
```{r}
res_pf$ensembl <- mapIds(Mus.musculus,
                     keys=row.names(res_pf),
                     column="ENSEMBL", #now add a column for gene names or gene description
                     keytype="ENSEMBL",
                     multiVals="first")
...
```{r}
res_pf$ENTREZID <- mapIds(Mus.musculus,
                     keys=row.names(res_pf),
                     column="ENTREZID", #now add a column for gene names or gene description
                     keytype="ENSEMBL",
                     multiVals="first")
...
```{r}
exp_resOrdered <- res_exp[order(res_exp$padj),]
pf_resOrdered <- res_pf[order(res_pf$padj),]
```
...
```{r}
tiff(file = "Figures/female_hippocampus_heatmap.tiff", units="in", width=8, height=5, res=1000)
heatmap<-pheatmap(mat, 
         annotation_col=df,
         labels_row = exp_resOrdered$symbol,
         fontsize = 6,
         scale = "row",
         show_colnames = F,
         cluster_cols = TRUE,
         main = "Female Hippocampus Heatmap")
heatmap
dev.off()
heatmap
```
...
#***volcano plots***
```{r}
tiff(file = 'Figures/volcano/Female_Exposed_volcano_plot_Hipp.tiff', units="in", width=8, height=5, res=1000)
M_vol1<- EnhancedVolcano(res_exp,
    lab = res_exp$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
    title = "Female Exposed Hippocampus",
    subtitle = "",
...
```{r}
exp_resOrdered <- as.data.frame(exp_resOrdered)
write_tsv(exp_resOrdered, file="Results_spreadsheets/exp_female_hip_results.tsv")
write_csv(exp_resOrdered, file="Results_spreadsheets/exp_female_hip_results.csv")
write_csv(as.data.frame(assay(F_se)), file= "Results_spreadsheets/female_hip_counts.csv")
```
